 ...
 * $ pg-to-ts generate -c postgresql://username:password@localhost:5432/tmf_app_manager -t action_plugin -t action_queue -t activity_log -t allowed_self_assignable_sections_shape -t application -t application_list_shape -t application_note -t application_response -t application_reviewer_action -t application_stage_history -t application_stage_status_all -t application_stage_status_latest -t application_status_history -t assigned_sections_by_stage_and_level -t constraints_info -t counter -t data_changelog -t data_table -t data_view -t data_view_column_definition -t element_type_plugin -t evaluator_fragment -t file -t filter -t grafana_dashboard_image -t notification -t organisation -t organisation_application_join -t permission_flattened -t permission_join -t permission_name -t permission_policy -t permissions_all -t postgres_row_level -t review -t review_assignment -t review_assignment_assigned_sections_shape -t review_assignment_assigner_join -t review_decision -t review_response -t review_status_history -t schema_columns -t system_info -t template -t template_action -t template_category -t template_data_view_join -t template_element -t template_file_join -t template_filter_join -t template_permission -t template_section -t template_stage -t template_stage_review_level -t trigger_queue -t trigger_schedule -t unnested_sections -t user -t user_application_join -t user_list -t user_list_admin -t user_org_join -t user_organisation -t verification -s public
 *
 */
...
const application = {
  ...
  columns: ['id', 'template_id', 'user_id', 'org_id', 'session_id', 'serial', 'name', 'outcome', 'is_active', 'is_config', 'trigger', 'outcome_registration', 'reviewer_list', 'assigner_list', 'url_properties'],
...
export interface ApplicationStageStatusAll {
  ...
  outcome: application_outcome | null;
  application_url_properties: Json | null;
}
...
const application_stage_status_all = {
  ...
  columns: ['application_id', 'template_id', 'template_name', 'template_code', 'serial', 'name', 'session_id', 'user_id', 'org_id', 'stage_id', 'stage_number', 'stage', 'stage_colour', 'stage_history_id', 'stage_history_time_created', 'stage_is_current', 'status_history_id', 'status', 'status_history_time_created', 'status_is_current', 'outcome', 'application_url_properties'],
  requiredForInsert: [],
  primaryKey: null,
  foreignKeys: {},
...
export interface EvaluatorFragment {
  id: number;
  code: string;
  name: string;
  description: string | null;
  expression: Json;
  front_end: boolean | null;
  back_end: boolean | null;
}
export interface EvaluatorFragmentInput {
  id?: number;
  code: string;
  name: string;
  description?: string | null;
...
const evaluator_fragment = {
  tableName: 'evaluator_fragment',
  columns: ['id', 'code', 'name', 'description', 'expression', 'front_end', 'back_end'],
  requiredForInsert: ['code', 'name', 'expression'],
  primaryKey: 'id',
  foreignKeys: {},
  $type: null as unknown as EvaluatorFragment,
  $input: null as unknown as EvaluatorFragmentInput
} as const;
...
const file = {
  tableName: 'file',
  columns: ['id', 'unique_id', 'original_filename', 'user_id', 'application_serial', 'application_response_id', 'description', 'application_note_id', 'is_output_doc', 'is_internal_reference_doc', 'is_external_reference_doc', 'to_be_deleted', 'file_path', 'thumbnail_path', 'mimetype', 'submitted', 'timestamp', 'archive_path', 'file_size', 'checksum', 'last_modified'],
...
// Table grafana_dashboard_image
export interface GrafanaDashboardImage {
  id: number;
  name: string;
  description: string | null;
  data: string | null;
...
export interface GrafanaDashboardImageInput {
  id?: number;
  name: string;
  description?: string | null;
  data?: string | null;
...
const grafana_dashboard_image = {
  tableName: 'grafana_dashboard_image',
  columns: ['id', 'name', 'description', 'data', 'updated_date'],
  requiredForInsert: ['name'],
  primaryKey: 'id',
  foreignKeys: {},
  $type: null as unknown as GrafanaDashboardImage,
  $input: null as unknown as GrafanaDashboardImageInput
} as const;
...
export interface Organisation {
  ...
  local_agent_business_address: string | null;
  license: Json | null;
  business_certificate: Json | null;
  tin_letter: Json | null;
  sub_type: string | null;
  other_documents: Json | null;
  is_sponsor_company: boolean | null;
  is_chemical_importer: boolean | null;
}
...
const organisation = {
  ...
  columns: ['id', 'name', 'registration', 'address', 'logo_url', 'is_system_org', 'registration_documentation', 'country', 'province', 'contact_email', 'contact_phone', 'postal_address', 'local_agent_name', 'local_agent_email', 'local_agent_phone_number', 'local_agent_business_address', 'license', 'business_certificate', 'tin_letter', 'sub_type', 'other_documents', 'is_sponsor_company', 'is_chemical_importer'],
  requiredForInsert: [],
  primaryKey: 'id',
  foreignKeys: {},
...
// Table template_file_join
export interface TemplateFileJoin {
  id: number;
  template_id: number;
  file_id: number;
}
export interface TemplateFileJoinInput {
  id?: number;
  template_id: number;
  file_id: number;
}
const template_file_join = {
  tableName: 'template_file_join',
  columns: ['id', 'template_id', 'file_id'],
  requiredForInsert: ['template_id', 'file_id'],
  primaryKey: 'id',
  foreignKeys: {
    template_id: { table: 'template', column: 'id', $type: null as unknown as Template },
    file_id: { table: 'file', column: 'id', $type: null as unknown as File },
  },
  $type: null as unknown as TemplateFileJoin,
  $input: null as unknown as TemplateFileJoinInput
} as const;
...
const template_permission = {
  ...
  requiredForInsert: ['permission_name_id', 'template_id'],
  primaryKey: 'id',
  foreignKeys: {
    permission_name_id: { table: 'permission_name', column: 'id', $type: null as unknown as PermissionName },
    template_id: { table: 'template', column: 'id', $type: null as unknown as Template },
...
export interface TableTypes {
  ...
  evaluator_fragment: {
    select: EvaluatorFragment;
    input: EvaluatorFragmentInput;
  };
  file: {
    select: File;
    input: FileInput;
  ...
  grafana_dashboard_image: {
    select: GrafanaDashboardImage;
    input: GrafanaDashboardImageInput;
  };
  notification: {
    select: Notification;
  ...
  template_file_join: {
    select: TemplateFileJoin;
    input: TemplateFileJoinInput;
  };
